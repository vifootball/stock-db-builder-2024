...
def copy_csv_file_to_db(csv_file_path, table_name, create_table_query):
    connection = get_db_connection()
    try:
        with connection: # connection 블록 내에서 모든 쿼리가 실행되므로, 모든 명령어가 성공하면 connection.commit()이 자동으로 호출
            with connection.cursor() as cursor:
                ...
                print(f"Upload data from {csv_file_path} to the table '{table_name}': Started")

                # COPY 명령어를 사용하여 CSV 파일을 통째로 테이블에 업로드
                with open(csv_file_path, 'r') as f:
                    cursor.copy_expert(f"COPY {table_name} FROM STDIN WITH CSV HEADER DELIMITER ','", f)

                print(f"Upload data from {csv_file_path} to the table '{table_name}': Ended")
